...
def maybe_prompt_existing_user_update(
    *,
    ...
    # [自研补丁] 冻结翻译环境更新 —— 始终返回, 不再弹出询问
    #
    # 背景: 原逻辑靠 `not sys.stdin.isatty()` 静默跳过(后台运行时无 tty),
    #       一旦前台运行 `python server.py` 排障, 就会开始询问 y/n。
    # 风险: 一旦回答 y, transactional_install_or_update() 会以 -I
    #       (--ignore-installed) 强制重装, 覆盖 site-packages 内的自研补丁;
    #       venv.json 的版本钉死(==1.9.11)只能防"升版本",
    #       防不住"同版本强制重装"。
    # 注意: 不可用 --enable_winexe 达成此目的 —— 该参数会让 server.py
    #       改用 winexe_path 指向的 pdf2zh.exe 执行翻译, 完全绕过 venv 补丁。
    # 恢复: 删除下方 return 即恢复上游行为。
    return
