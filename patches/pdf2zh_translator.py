...
class OpenAITranslator(BaseTranslator):
    ...
    def _clean_cjk_typography(text: str) -> str:
        ...
        c = re.sub(r"（(?:\s*（)+", "（", c)
        c = re.sub(r"）(?:\s*）)+", "）", c)
        # [自研补丁 2026-09-12 v26.4] ⑤ 裸花括号脱壳: 源文本里的真花括号已被
        # converter 的 vchar 字符表保护成 {vN} 占位符, 故译文中"非 {vN}"的
        # 花括号必是 LLM 模仿占位符语法自造的伪影(实例: Cactaceae 参考文献页
        # 把术语写成 "{仙人掌科}"×7, 纯文本渲染出可见花括号)。脱壳保留内容,
        # {vN} 原样保留 → 占位符多重集与 polish 锚点断言不受影响。
        if "{" in c or "}" in c:
            c = "".join(
                p if re.fullmatch(r"\{v\d+\}", p) else p.replace("{", "").replace("}", "")
                for p in re.split(r"(\{v\d+\})", c)
            )
        return c
