...
# 老式 Type1 字体的差异编码 glyph 名 /C{xx}: xx 是 Adobe StandardEncoding 码
_CNAME = re.compile(r"C(\d+)")
            ...
            for x in diff:
                if isinstance(x, int):
                    ...
                    try:
                        ...
                        # C{xx} glyph 名无法按 AGL 解析时, xx 即 Adobe StandardEncoding 码
                        m = _CNAME.match(cast(str, x.name))
                        if m:
                            code = int(m.group(1))
                            if code < 128:
                                cid2unicode[cid] = chr(code)
                            elif code in cls.std2unicode:
                                cid2unicode[cid] = cls.std2unicode[code]
                            else:
                                log.debug(str(e))
                        else:
                            log.debug(str(e))
