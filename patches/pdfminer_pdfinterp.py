...
class PDFResourceManager:
    ...
    def get_font(self, objid: object, spec: Mapping[str, object]) -> PDFFont:
        if objid and objid in self._cached_fonts:
            ...
            # [自研补丁 2026-09-18 闸门3] 字体构造兜底：原文这里没有 try，
            # 任何字体在构造期抛异常都会冒到顶层 → 单个坏字体炸掉整篇 PDF
            # （v26.19 的 Type3 崩溃即此，pdf2zh 退出码 1、产物全无）。
            # 现在就地降级为 NullFont 并**留痕**（台账 + warning），
            # 把"整篇失败"换成"该字体覆盖的那点文字没有译文，页面其余照常"。
            # STRICT 模式保持原语义（严格模式就是要炸），不受本补丁影响。
            try:
                font = self._create_font(objid, spec)
            except Exception as exc:
                if settings.STRICT:
                    raise
                rec = record_font_fallback(objid, spec, exc)
                log.warning(
                    "font fallback -> NullFont: objid=%r subtype=%s basefont=%r (%s @ %s)",
                    rec["objid"], rec["subtype"], rec["basefont"],
                    rec["err"], rec["where"])
                font = PDFNullFont(fallback_of=rec["subtype"])
            if objid and self.caching:
                self._cached_fonts[objid] = font
        return font

    def _create_font(self, objid: object, spec: Mapping[str, object]) -> PDFFont:
        """[自研补丁] 原 get_font 的构造体，整体搬进这里，行为不变"""
        if settings.STRICT:
