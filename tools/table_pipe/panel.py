...
$('rvCopyBtn').addEventListener('click',function(){
  ...
  api('/api/copy',{text:rvText(r)}).then(function(x){
    ...
    log('已把 '+r.items.length+' 条存疑复制到剪贴板('+x.n_chars+' 字符); 粘给'+vendor+', '
        +'明确要求「只改点到的段, 其余照抄」。');
  });
});
...
function doCommit(){
  ...
  api('/api/commit',{text:ta.value}).then(function(r){
    ...
    setBanner(r.warn?'warn':'ok',
              (r.warn?'! 已出稿, 但这一篇还有没出稿的: ':'✓ 已出稿: ')+r.out);
...
class H(BaseHTTPRequestHandler):
    ...
    def _review(self, b):
        ...
        if "error" in r:
            ...
            return
        self._json({"ok": True, "job": r["job"], "n": r["n_units"], "vendor": r["vendor"],
    ...
    def _commit(self, b):
        ...
        self._json({"ok": True, "log": logs, "out": out, "todos": paper_tasks(),
                    "warn": names if miss else ""})
