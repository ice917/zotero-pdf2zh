...
定位 [v26]: 军师是"发现者"而非"修复者"。四类问题里只有一部分能在缓存层就地修复
(最小编辑 find/replace 写回); 跨页断句这类问题**在缓存层没有修复出口**(段本地字符串
改写无法跨页重排 —— 流式解析下页 N 渲染时页 N+1 尚未解析), 正确产出形态就是"准确的
待手术清单"(reports)。因此不要用"建议自动可用率"评价军师, 恰当指标是**清单准确率**。
Cactaceae 实测 (154 段/34 页): 断言断裂 7 条全部属实 (精确率 7/7), 另有 2 条实为断开
却被判"完整" (召回 7/9) —— 清单必须人工过一遍再动手。
